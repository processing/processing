...
public class PTexture implements PConstants { 
  ...
  protected PGraphicsLWJGL pg;    // The main renderer
  ...
  public void set(int[] pixels, int x, int y, int w, int h, int format) {
    ...
    if (usingMipmaps) {
      if (PGraphicsLWJGL.mipmapGeneration) {
  ...
  protected void setSize(int w, int h) {
    ...
    if (PGraphicsLWJGL.npotTexSupported) {
      glWidth = w;
      glHeight = h;
    } else {
    ...
    if ((glWidth > PGraphicsLWJGL.maxTextureSize) || (glHeight > PGraphicsLWJGL.maxTextureSize)) {
      glWidth = glHeight = 0;
      throw new RuntimeException("Image width and height cannot be" +
                                 " larger than " + PGraphicsLWJGL.maxTextureSize +
