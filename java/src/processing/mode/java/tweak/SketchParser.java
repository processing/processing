...
public class SketchParser {
	...
	public SketchParser(String[] codeTabs, boolean requiresComment) {
		...
		for (int tabID = 0; tabID < codeTabs.length; tabID++) {
			String code = codeTabs[tabID];
			commentBlocks.add(getCommentBlocks(code, tabID));
		}

		// add 'settings' and 'setup' to ignore list (to ignore all numbers
		// there)
		ignoreFunctions = new ArrayList<Range>();
		ignoreFunctions.add(new Range(getVoidFunctionStart(codeTabs[0],
				"settings"), getVoidFunctionEnd(codeTabs[0], 0, "settings"), 0));
		ignoreFunctions.add(new Range(
				getVoidFunctionStart(codeTabs[0], "setup"), getVoidFunctionEnd(
						codeTabs[0], 0, "setup"), 0));

		// build curly scope for every character in the code
		curlyScopes = new ArrayList<>();
		for (int tabID = 0; tabID < codeTabs.length; tabID++) {
			String code = codeTabs[tabID];
			curlyScopes.add(getCurlyScopes(code, tabID));
		}
	...
	private void addAllDecimalNumbers() {
		...
		System.out.println("parsing all the decimals!");
		...
		for (int i = 0; i < codeTabs.length; i++) {
			...
			System.out.println("In Tab " + i);

			while (m.find()) {
				boolean forceFloat = false;
				int start = m.start() + 1;
				int end = m.end();
				System.out.println("Found " + c.substring(start, end));

				if (isInRangeList(start, i, commentBlocks.get(i))) {
					// ignore comments
					System.out.println("Skipping because of comment");
					continue;
				}

				if (isInRangeList(start, i, ignoreFunctions)) {
					// ignore numbers in predefined functions
					System.out
							.println("Skipping because part of function name");
					continue;
				}

				if (requiresComment) {
					// only add numbers that have the "// tweak" comment in
					// their line
					if (!lineHasTweakComment(start, c)) {
						System.out
								.println("Skipping because of no tweak comment");
						continue;
					}
				}
				...
				for (Range r : scientificNotations.get(i)) {
					if (r.contains(start, i)) {
						found = true;
						System.out
								.println("Skipping because of scientific notation");
						break;
					}
				}
				if (found) {
				...
				if (value.contains(".") || forceFloat) {
					...
					handles.add(new Handle("float", name, floatVarCount, value,
							i, line, start, end, decimalDigits));
					floatVarCount++;
				} else {
					// consider this as an int
					String name = varPrefix + "_int[" + intVarCount + "]";
					handles.add(new Handle("int", name, intVarCount, value, i,
							line, start, end, 0));
					intVarCount++;
				}
			}
	...
	private void addAllHexNumbers() {
		...
		for (int i = 0; i < codeTabs.length; i++) {
			...
			while (m.find()) {
				...
				Handle handle;
				try {
					handle = new Handle("hex", name, intVarCount, value, i,
							line, start, end, 0);
	...
	/**
	 * Get a list of all the webcolors (#) numbers in the code list of all
	 * hexadecimal numbers in the sketch
	 */
	private void addAllWebColorNumbers() {
		...
		for (int i = 0; i < codeTabs.length; i++) {
			...
			while (m.find()) {
				...
				Handle handle;
				try {
					handle = new Handle("webcolor", name, intVarCount, value,
							i, line, start, end, 0);
				} catch (NumberFormatException e) {
					// don't add this number
					continue;
	...
	private ArrayList<ColorMode> findAllColorModes() {
		...
		for (int i = 0; i < codeTabs.length; i++) {
			...
			while ((index = tab.indexOf("colorMode", index + 1)) > -1) {
				...
				if (isInRangeList(index, i, commentBlocks.get(i))) {
					// ignore comments
					continue;
				}
	...
	private void createColorBoxes() {
		...
		// search tab for the functions: 'color', 'fill', 'stroke',
		// 'background', 'tint'
		Pattern p = Pattern
				.compile("color\\(|color\\s\\(|fill[\\(\\s]|stroke[\\(\\s]|background[\\(\\s]|tint[\\(\\s]");
		...
		for (int i = 0; i < codeTabs.length; i++) {
			...
			while (m.find()) {
				...
				if (isInRangeList(m.start(), i, commentBlocks.get(i))) {
					// ignore colors in a comment
					continue;
				}

				if (isInRangeList(m.start(), i, ignoreFunctions)) {
					// ignore numbers in predefined functions
					continue;
				}
				...
				if (colorHandles.size() > 0) {
					/*
					 * make sure there is no other stuff between '()' like
					 * variables. subtract all handle values from string inside
					 * parenthesis and check there is no garbage left
					 */
					...
					for (int j = 0; j < insidePar.length(); j++) {
						if (insidePar.charAt(j) != ' '
								&& insidePar.charAt(j) != ',') {
							// don't add this color box because we can not know
							// the
					...
					if (!garbage) {
						// find the context of the color (e.g. this.fill() or
						// <object>.fill())
						String context = getObject(m.start(), tab);
						ColorMode cmode = getColorModeForContext(context);

						// not adding color operations for modes we couldn't
						// understand
						ColorControlBox newCCB = new ColorControlBox(context,
								cmode, colorHandles);

						if (cmode.unrecognizedMode) {
							// the color mode is unrecognizable add only if is a
							// hex or webcolor
							if (newCCB.isHex) {
								colorBox.add(newCCB);
							}
	...
	private void createColorBoxesForLights() {
		...
		Pattern p = Pattern
				.compile("ambientLight[\\(\\s]|directionalLight[\\(\\s]"
		...
		for (int i = 0; i < codeTabs.length; i++) {
			...
			while (m.find()) {
				...
				// put 'colorParamsEnd' after three parameters inside the
				// parenthesis or at the close
				...
				if (colorHandles.size() > 0) {
					 ...
					 * parenthesis and check there is no garbage left
					 */
					String insidePar = tab.substring(openPar + 1,
							colorParamsEnd);
					for (Handle h : colorHandles) {
						insidePar = insidePar.replaceFirst(h.strValue, "");
					}
	...
	private ColorMode getColorModeForContext(String context) {
		...
		// if none found, create the default color mode for this context and
		// return it
	...
	private List<List<Range>> getAllScientificNotations() {
		...
		for (int tabID = 0; tabID < codeTabs.length; tabID++) {
			...
			while (m.find()) {
				notation.add(new Range(m.start(), m.end(), tabID));
			}
	...
	static private boolean hasTweakComment(String code) {
		...
		/*
		 * Pattern p = Pattern.compile("\\/\\/.*tweak",
		 * Pattern.CASE_INSENSITIVE); Matcher m = p.matcher(code); return
		 * m.find();
		 */
	}
	...
	/**
	 * Builds an int array for every tab that represents the scope depth at each
	 * character.
	 */
	static private int[] getCurlyScopes(String code, int tabID) {
		List<Range> comments = getCommentBlocks(code, tabID);
		...
		for (int pos = 0; pos < code.length(); pos++) {
			...
			if (isInRangeList(pos, tabID, comments)) {
	 ...
	 *            position
	 * @param codeTabIndex
	 *            index of the code in codeTabs
	 * @return true if the position 'pos' is in global scope in the code
	 *         'codeTabs[codeTabIndex]'
	 * 
	 */
	...
	static private List<Range> getCommentBlocks(String code, int tabID) {
		...
		for (int pos = 0; pos < code.length() - 1; pos++) {
			if (lookForEnd) {
				...
				if (code.charAt(pos) == '*' && code.charAt(pos + 1) == '/') {
					commentBlocks.add(new Range(lastBlockStart, pos + 1, tabID));
					lookForEnd = false;
				}
			} else {
				if (code.charAt(pos) == '/' && code.charAt(pos + 1) == '*') {
					...
					commentBlocks.add(new Range(pos, getEndOfLine(pos, code), tabID));
				}
			}
	...
	static private boolean isInRangeList(int pos, int tabID, List<Range> rangeList) {
		for (Range r : rangeList) {
			if (r.contains(pos, tabID)) {
				return true;
			}
		}
		return false;
	...
	/**
	 * returns the object name (what comes before the '.') of the function
	 * starting at 'pos'
	 */
	...
	static public int getVoidFunctionEnd(String code, int tabID, String functionName) {
		List<Range> comments = getCommentBlocks(code, tabID);
	...
	static public int getSetupEnd(String code, int tabID) {
		List<Range> comments = getCommentBlocks(code, tabID);
	...
	static public int getAfterSizePos(String code, int tabID) {
		List<Range> comments = getCommentBlocks(code, tabID);
		...
		while (m.find()) {
			if (isInRangeList(m.start(), tabID, comments)
					|| isInRangeList(m.end(), tabID, comments)) {
			...
			while (pos < code.length()) {
				if (code.charAt(pos) == ';' && !isInRangeList(pos, tabID, comments)) {
					found = true;
					break;
				}
				pos++;
	...
	static class Range {
		...
		boolean contains(int v, int t) {
			return v >= start && v < end && tab == t;
		}
	}
}
