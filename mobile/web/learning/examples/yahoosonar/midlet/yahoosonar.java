  ...
  if (library == parser) {
    if (event == XMLParser.EVENT_TAG_START) {      
      ...
      if (data.equals(TAG_RESULTSET)) {
        numResults = PMIDlet.toInt(parser.attribute(ATTR_RESULTS));
      ...
      if (lastTag.equals(TAG_DISTANCE)) {
        ...
        if (index >= 0) {
          results[numResults].distance = PMIDlet.toInt(value.substring(index + 1));
          value = value.substring(0, index);
        }
        results[numResults].distance += 100 * PMIDlet.toInt(value);
