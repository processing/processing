...
public class PFont implements PConstants {
  ...
  public void save(OutputStream output) throws IOException {
    ...
    for (int i = 0; i < charCount; i++) {
      ...
      os.writeByte(height[i]);
      os.writeByte(width[i]);
      os.writeByte(setWidth[i]);
      os.writeByte(topExtent[i]);
      os.writeByte(leftExtent[i]);
    }
  ...
  */
  ...
  static {
    DEFAULT_CHARSET = new char[126-33+1 + 172-161+1 + 255-174+1];
    int index = 0;
 ...
 */
  ...
  public PFont(Font font, boolean smooth, char charset[]) {
    ...
    try {
      ...
      for (int i = 255; i >= 0; i--) {
          rs[i] = gs[i] = bs[i] = (byte) (255 - i);
      }
      IndexColorModel cm = new IndexColorModel(8, 256, rs, gs, bs, 0);      
      imgs = new BufferedImage[charCount];
    ...
    for (int i = 0; i < charCount; i++) {
      ...
      */
      ...
      //// draw character into indexed image buffer for export as png
      imgs[i] = new BufferedImage(width[index], height[index], BufferedImage.TYPE_BYTE_INDEXED, cm);
      Graphics2D g2 = (Graphics2D) imgs[i].getGraphics();
      g2.setRenderingHint((RenderingHints.Key) antialiasingKey, antialiasState);
      g2.setFont(font);
