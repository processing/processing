...
*/
    ...
    try {
//// mobile: set base class and imports        
...
*/
  ...
  public boolean exportMIDlet() throws Exception {
    ...
    File midletDir = new File(folder, "midlet");

    boolean writeJad = true;
    if (midletDir.exists()) {
      File jadFile = new File(midletDir, name + ".jad");
      if (jadFile.exists() && !replaceJad) {
        writeJad = false;
      }
    ...
    //// generate manifest file and JAD file
    File mfOutputFile = new File(tmpDir, "MANIFEST.MF");
    FileOutputStream fos = new FileOutputStream(mfOutputFile);
    PrintStream ps = new PrintStream(fos);
    
    File jadFile = new File(midletDir, name + ".jad");
    PrintStream jadPs = new PrintStream(new FileOutputStream(jadFile));

    InputStream is = null;
    // if there is a mobile.mf file in the sketch folder, use that
    File customMf = new File(folder, "mobile.mf");
    if (customMf.exists()) {
    ...
    // remove the .class files from the midlet folder
    classfiles = midletDir.list();
    for (int i = 0; i < classfiles.length; i++) {
      if (classfiles[i].endsWith(".class")) {
