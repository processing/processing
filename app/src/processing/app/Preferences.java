...
//    fontSizeField.setSelectedItem(editorFont.getSize());
    ...
    presentColorHex.setEditable(false);
    presentColorHex.setBackground(Color.WHITE);
    
    final CustomColorChooser c = new CustomColorChooser(Preferences.getColor("run.present.bgcolor"),
    		presentColorHex, presentColor);
	  
    presentColorHex.addMouseListener(new MouseListener() {
...
//    top += d.height + GUI_BETWEEN;
  ...
  private class CustomColorChooser { 
	  ...
	  public CustomColorChooser(Color initialColor, final JTextField externalHex, 
	    ...
	    window = new JDialog(owner, "Choose the background color for Present", true);
	  ...
	  public void hide() {
	    ...
	    setColor(new Color(Integer.parseInt(externalHex.getText().substring(0, 2), 16), 
	    		Integer.parseInt(externalHex.getText().substring(2, 4), 16), 
	    		Integer.parseInt(externalHex.getText().substring(4, 6), 16)));
	  }
	  ...
	  protected Container createColorFields(String buttonName, ActionListener buttonListener,
			  String buttonName2, ActionListener buttonListener2) {
