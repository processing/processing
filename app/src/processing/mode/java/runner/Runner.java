...
//      System.out.println("actually it's a " + listener.getClass().getName());
    ...
    for (Library library : build.getImportedLibraries()) {
      if (!library.supportsArch(PUtil.platform, bits)) {
...
//    String debugArg = "-Xdebug";
    ...
    commandArgs = PUtil.concat(commandArgs, vmParams);
    commandArgs = PUtil.concat(commandArgs, sketchParams);
//  PUtil.println(commandArgs);
//  commandArg.setValue(commandArgs);
    ...
    //PUtil.println(connector);  // gets the defaults
...
//      throw new Error("Unable to launch target VM: " + exc);
  ...
  protected String[] getMachineParams() {
    ...
    if (options.length() > 0) {
      String pieces[] = PUtil.split(options, ' ');
...
//        Base.librariesClassPath);
    ...
    params.add("-ea");
    //PUtil.println(PUtil.split(sketch.classPath, ':'));

    String outgoing[] = new String[params.size()];
    params.toArray(outgoing);

//    PUtil.println(outgoing);
//    PUtil.println(PUtil.split(outgoing[0], ":"));
//    PUtil.println();
//    PUtil.println("class path");
//    PUtil.println(PUtil.split(outgoing[2], ":"));
...
//  PUtil.println(PUtil.split(Base.librariesClassPath, ';'));
//  System.out.println();
  }
...
//    return outgoing;
  ...
  protected void launchJava(final String[] args) {
    new Thread(new Runnable() {
      public void run() {
//        PUtil.println("java starting");
        process = PUtil.exec(args);
        try {
//          PUtil.println("java waiting");
          int result = process.waitFor();
//          PUtil.println("java done waiting");
          if (result != 0) {
...
//            PUtil.println("launchJava stderr:");
//            PUtil.println(errorStrings);
//            PUtil.println("launchJava stdout:");
            PUtil.printArray(inputStrings);
            ...
            if (errorStrings != null && errorStrings.length > 1) {
              if (errorStrings[0].indexOf("Invalid maximum heap size") != -1) {
                ...
                System.err.println("Using startup command: " + PUtil.join(args, " "));
              }
            } else {
              //exc.printStackTrace();
